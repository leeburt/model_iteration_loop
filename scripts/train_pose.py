...
from yolo_iter.training import train_yolo


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Train YOLO model from YAML config.")
...
def main() -> None:
    ...
    run_name = str(cfg.get("name", "train_yolo"))
    log_file = Path(args.log_dir) / f"{run_name}.log"
    logger = setup_logger("train_yolo", log_file)
